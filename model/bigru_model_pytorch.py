...
class SentimentGRU(nn.Module,BaseModel):
    """
    ...
    def __init__(self,TrainData, ValidationData, embedding_type , embedding_dim,  hidden_dim, output_size, batch_size, n_layers, drop_prob=0.5,Device=torch.device("cpu")):
        ...
        # LSTM Layer
        self.gru = nn.GRU(self.embedding_dim, hidden_dim, n_layers,dropout=drop_prob, batch_first=True,bidirectional=True)
    ...
    def forward(self, input, hidden):
        ...
        gru_out = torch.cat((hidden[-2, :, :], hidden[-1, :, :]), dim=1)

        gru_out = self.decoder1(gru_out)

        gru_out = self.relu(gru_out)

        gru_out = self.dropout(gru_out)

        gru_out = self.decoder2(gru_out)

        return F.log_softmax(gru_out,dim=1),hidden
    ...
    def build(cls,TrainData, ValidationData,embedding_type , embedding_dim, hidden_dim, output_dim, batch_size, n_layers,dropout_prob,Device):
      cls.model = SentimentGRU(TrainData, ValidationData,embedding_type,embedding_dim, hidden_dim, output_dim, batch_size, n_layers,dropout_prob,Device)
      return cls.model
        ...
        for e in range(epochs):
            ...
            for itere in range(0,self.val_df.shape[0],self.batch_size):
                if self.val_df[itere:itere+self.batch_size].shape[0]%self.batch_size == 0:
                    ...
                    valid_hidden = valid_hidden.data
